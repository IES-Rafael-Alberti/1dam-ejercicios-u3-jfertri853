...
def sacar_domicilio(compra: tuple) -> str:
    return compra[3]


def obtener_domicilios(lista_compras: list) -> set:
    return set(sacar_domicilio(compra) for compra in lista_compras)


def mostrar_domicilios(domicilios: set):
    print("Hay que mandar facturas a:")
...
def main():
    ...
    domicilios = obtener_domicilios(compras)
